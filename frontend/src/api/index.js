class Api {
  ...
  getRecipes ({
    ...
    tags
      ...
      return fetch(
        `/api/v1/recipes/?page=${page}&limit=${limit}${author ? `&author=${author}` : ''}${is_favorited ? `&is_favorited=${is_favorited}` : ''}${is_in_shopping_cart ? `&is_in_shopping_cart=${is_in_shopping_cart}` : ''}${tagsString}`,
        {
          method: 'GET',
          headers: {
            ...this._headers,
  ...
  getSubscriptions ({
    ...
    const token = localStorage.getItem('token')
    return fetch(
      `/api/v1/users/subscriptions/?page=${page}&limit=${limit}&recipes_limit=${recipes_limit}`,
      {
        method: 'GET',
        headers: {
          ...this._headers,
  ...
  deleteSubscriptions ({
    ...
    const token = localStorage.getItem('token')
    return fetch(
      `/api/v1/users/${author_id}/subscribe/`,
      {
        method: 'DELETE',
        headers: {
          ...this._headers,
  ...
  addToOrders ({ id }) {
    const token = localStorage.getItem('token')
    return fetch(
      `/api/v1/recipes/${id}/shopping_cart/`,
      {
        method: 'POST',
        headers: {
          ...this._headers,
